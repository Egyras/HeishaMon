...
void getWifiScanResults(int numSsid)
{
  ...
  { // found wifi networks
    ...
    { // fill the sorted list with normal indexes first
      indexes[i] = i;
    }
    for (int i = 0; i < numSsid; i++)
    { // then sort
    ...
    if (LittleFS.exists("/config.json"))
    {
      ...
      if (configFile)
      {
        ...
        if (!error)
        {
          ...
          if (jsonDoc["wifi_ssid"])
            strncpy(heishamonSettings->wifi_ssid, jsonDoc["wifi_ssid"], sizeof(heishamonSettings->wifi_ssid));
          if (jsonDoc["wifi_password"])
            strncpy(heishamonSettings->wifi_password, jsonDoc["wifi_password"], sizeof(heishamonSettings->wifi_password));
          if (jsonDoc["wifi_hostname"])
            strncpy(heishamonSettings->wifi_hostname, jsonDoc["wifi_hostname"], sizeof(heishamonSettings->wifi_hostname));
          if (jsonDoc["ota_password"])
            strncpy(heishamonSettings->ota_password, jsonDoc["ota_password"], sizeof(heishamonSettings->ota_password));
          if (jsonDoc["mqtt_topic_base"])
            strncpy(heishamonSettings->mqtt_topic_base, jsonDoc["mqtt_topic_base"], sizeof(heishamonSettings->mqtt_topic_base));
          if (jsonDoc["mqtt_server"])
            strncpy(heishamonSettings->mqtt_server, jsonDoc["mqtt_server"], sizeof(heishamonSettings->mqtt_server));
          if (jsonDoc["mqtt_port"])
            strncpy(heishamonSettings->mqtt_port, jsonDoc["mqtt_port"], sizeof(heishamonSettings->mqtt_port));
          if (jsonDoc["mqtt_username"])
            strncpy(heishamonSettings->mqtt_username, jsonDoc["mqtt_username"], sizeof(heishamonSettings->mqtt_username));
          if (jsonDoc["mqtt_password"])
            strncpy(heishamonSettings->mqtt_password, jsonDoc["mqtt_password"], sizeof(heishamonSettings->mqtt_password));
          if (jsonDoc["ntp_servers"])
            strncpy(heishamonSettings->ntp_servers, jsonDoc["ntp_servers"], sizeof(heishamonSettings->ntp_servers));
          if (jsonDoc["timezone"])
            heishamonSettings->timezone = jsonDoc["timezone"];
          ...
          if (jsonDoc["waitTime"])
            heishamonSettings->waitTime = jsonDoc["waitTime"];
          if (heishamonSettings->waitTime < 5)
            heishamonSettings->waitTime = 5;
          if (jsonDoc["waitDallasTime"])
            heishamonSettings->waitDallasTime = jsonDoc["waitDallasTime"];
          if (heishamonSettings->waitDallasTime < 5)
            heishamonSettings->waitDallasTime = 5;
          if (jsonDoc["dallasResolution"])
            heishamonSettings->dallasResolution = jsonDoc["dallasResolution"];
          if ((heishamonSettings->dallasResolution < 9) || (heishamonSettings->dallasResolution > 12))
            heishamonSettings->dallasResolution = 12;
          if (jsonDoc["updateAllTime"])
            heishamonSettings->updateAllTime = jsonDoc["updateAllTime"];
          if (heishamonSettings->updateAllTime < heishamonSettings->waitTime)
            heishamonSettings->updateAllTime = heishamonSettings->waitTime;
          if (jsonDoc["updataAllDallasTime"])
            heishamonSettings->updataAllDallasTime = jsonDoc["updataAllDallasTime"];
          if (heishamonSettings->updataAllDallasTime < heishamonSettings->waitDallasTime)
            heishamonSettings->updataAllDallasTime = heishamonSettings->waitDallasTime;
          if (jsonDoc["s0_1_gpio"])
            heishamonSettings->s0Settings[0].gpiopin = jsonDoc["s0_1_gpio"];
          if (jsonDoc["s0_1_ppkwh"])
            heishamonSettings->s0Settings[0].ppkwh = jsonDoc["s0_1_ppkwh"];
          if (jsonDoc["s0_1_interval"])
            heishamonSettings->s0Settings[0].lowerPowerInterval = jsonDoc["s0_1_interval"];
          if (jsonDoc["s0_1_minpulsewidth"])
            heishamonSettings->s0Settings[0].minimalPulseWidth = jsonDoc["s0_1_minpulsewidth"];
          if (jsonDoc["s0_1_maxpulsewidth"])
            heishamonSettings->s0Settings[0].maximalPulseWidth = jsonDoc["s0_1_maxpulsewidth"];
          if (jsonDoc["s0_2_gpio"])
            heishamonSettings->s0Settings[1].gpiopin = jsonDoc["s0_2_gpio"];
          if (jsonDoc["s0_2_ppkwh"])
            heishamonSettings->s0Settings[1].ppkwh = jsonDoc["s0_2_ppkwh"];
          if (jsonDoc["s0_2_interval"])
            heishamonSettings->s0Settings[1].lowerPowerInterval = jsonDoc["s0_2_interval"];
          if (jsonDoc["s0_2_minpulsewidth"])
            heishamonSettings->s0Settings[1].minimalPulseWidth = jsonDoc["s0_2_minpulsewidth"];
          if (jsonDoc["s0_2_maxpulsewidth"])
            heishamonSettings->s0Settings[1].maximalPulseWidth = jsonDoc["s0_2_maxpulsewidth"];
...
int getSettings(struct webserver_t *client, settingsStruct *heishamonSettings)
{
  ...
  case 10:
  {
    ...
    if (heishamonSettings->s0Settings[i].gpiopin == 255)
      heishamonSettings->s0Settings[i].gpiopin = DEFAULT_S0_PIN_1; // dirty hack
  ...
  case 11:
  {
    ...
    if (heishamonSettings->s0Settings[i].gpiopin == 255)
      heishamonSettings->s0Settings[i].gpiopin = DEFAULT_S0_PIN_2; // dirty hack
    itoa(heishamonSettings->s0Settings[i].gpiopin, str, 10);
    webserver_send_content(client, str, strlen(str));
...
int handleRoot(struct webserver_t *client, float readpercentage, int mqttReconnects, settingsStruct *heishamonSettings)
{
  ...
  case 3:
  {
    String resetReason = ESP.getResetInfo();
    webserver_send_content(client, (char *)resetReason.c_str(), resetReason.length());
      ...
      for (uint8_t topic = client->content; topic < HEATPUMP_TOPIC_Last && topic < client->content + 4; topic++)
      {
...

        {
          ...
          case DECODE_RESULT_INT:
            dataValue = String(result.int_value);
            break;
          case DECODE_RESULT_FLOAT:
            dataValue = String(result.float_value);
            break;
          case DECODE_RESULT_STRING:
            dataValue = String(result.string_value);
            break;
          }
...
int handleJsonOutput(struct webserver_t *client, char *serial_decoder_buffer)
{
  ...
  {
    for (uint8_t topic = client->content - 1; topic < HEATPUMP_TOPIC_Last && topic < client->content + 4; topic++)
    {
      ...
      if (topic < HEATPUMP_TOPIC_Last - 1)
      {
        webserver_send_content_P(client, PSTR(","), 1);
      }
    }
    ...
    if (client->content > HEATPUMP_TOPIC_Last)
    {
      client->content = HEATPUMP_TOPIC_Last;
    }
  }
  else if (client->content == HEATPUMP_TOPIC_Last + 1)
  {
    webserver_send_content_P(client, PSTR("],\"1wire\":"), 10);

    dallasJsonOutput(client);
  }
  else if (client->content == HEATPUMP_TOPIC_Last + 2)
  {
