...
void initDallasSensors(void (*log_message)(char *), unsigned int updateAllDallasTimeSettings, unsigned int dallasTimerWaitSettings, unsigned int dallasResolution)
{
  ...
  sprintf_P(log_msg, PSTR("Number of 1wire sensors on bus: %d"), dallasDevicecount);
  log_message(log_msg);
  ...
  for (int i = 0; i < dallasDevicecount; i++)
  {
    ...
    sprintf_P(log_msg, PSTR("Found 1wire sensor: %s"), actDallasData[i].address);
    log_message(log_msg);
  }
  if (DALLASASYNC)
...
void readNewDallasTemp(PubSubClient &mqtt_client, void (*log_message)(char *), char *mqtt_topic_base)
{
  ...
  for (int i = 0; i < dallasDevicecount; i++)
  {
    ...
    if (temp < -120.0)
    {
      sprintf_P(log_msg, PSTR("Error 1wire sensor offline: %s"), actDallasData[i].address);
      log_message(log_msg);
    }
    else
    {
      ...
      else
      {
        ...
        if ((updatenow) || (actDallasData[i].temperature != temp))
        { // only update mqtt topic if temp changed or after each update timer
          ...
          sprintf_P(mqtt_topic, PSTR("%s/%s/%s"), mqtt_topic_base, mqtt_topic_1wire, actDallasData[i].address);
